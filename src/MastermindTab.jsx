...
import { Lock, KeyRound, Eye, Lightbulb, Plus, Trash2, Key, Target, Sparkles } from 'lucide-react';
...
export default function MastermindTab({ bgCard, bgInput }) {
  ...
  const [vaultType, setVaultType] = useState('plan');
  ...
  // شاشة القفل المحسنة والمضبوطة بدون مشاكل الـ Fullscreen
  if (!isAuthenticated) {
    return (
      <div className="w-full min-h-[70vh] flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-slate-900 rounded-3xl p-8 border-4 border-emerald-500 shadow-[0_0_50px_rgba(16,185,129,0.3)] text-center relative overflow-hidden">
          ...
          <div className="inline-flex p-4 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 mb-4">
            <Lock size={36} />
          </div>

          <h2 className="text-2xl font-black text-white mb-1 tracking-wide">غرفة العقل المدبر</h2>
          <p className="text-xs text-emerald-400 font-semibold mb-6">منطقة سيادية تشفيرية - أدخل الرمز السري</p>
  ...
  return (
    <div className="relative space-y-6 min-h-screen text-slate-100">
      <div className={`p-6 rounded-3xl border ${bgCard} shadow-2xl border-amber-500/30 bg-gradient-to-r from-slate-900/90 via-slate-900/40 to-slate-900/90 backdrop-blur-xl`}>
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/40 text-amber-400">
              <Eye size={36} />
            </div>
            <div>
              <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 px-3 py-1 rounded-full text-amber-400 text-xs font-semibold mb-1">
                <Sparkles size={14} /> غرفة التفكير الاستراتيجي
              </div>
              <h2 className="text-2xl md:text-3xl font-black text-amber-400">العقل المدبر - Mastermind Vault</h2>
              <p className="text-xs text-slate-400">حفظ الخطط العلياً والبيانات والحسابات الحساسة.</p>
            </div>
          </div>
      ...
      <div className={`p-6 rounded-3xl border ${bgCard} shadow-xl border-amber-500/20 space-y-4`}>
        ...
        <form onSubmit={handleAddItem} className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <input 
            type="text"
            placeholder={vaultType === 'plan' ? "عنوان الفكرة / المشروع" : "اسم الخدمة / المنصة"}
            value={title}
          ...
          <div className="md:col-span-3">
            <button type="submit" className="bg-amber-500 text-slate-950 px-6 py-2.5 rounded-xl font-bold text-xs hover:bg-amber-400 transition inline-flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map((item) => (
          <div key={item.id} className={`p-5 rounded-3xl border ${bgCard} border-slate-700/50 space-y-3 shadow-lg hover:border-amber-500/40 transition`}>
